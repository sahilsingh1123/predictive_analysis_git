...
from PredictionAlgorithms.RidgeRegression import Ridge_reg
...
def root():
    ...
    try:
        if algo_name == "linear_reg":
            responseData = LinearRegressionModel(trainDataRatio=trainDataPercentage).linearReg(dataset_add=fileLocation, feature_colm=feature_colm_req, label_colm=label_colm_req, relation_list= relation_list, relation=relation, userId = modelId)
...
if (__name__=='__main__'):
    ...
    app.run(host='10.171.0.173', port = 3334, debug=False)
