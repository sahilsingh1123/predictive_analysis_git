...
class ArimaForecasting():
    def arimaForecasting(self):
        dataset = pd.read_csv("/home/fidel/mltest/BI.csv", infer_datetime_format=True, header=0, delimiter=",")
        ...
        plt.title('RSS: %.4f' % sum((results_ARIMA.fittedvalues - indexedDataset[targetColm]) ** 2))
        plt.show()
        ...
        predictions_ARIMA_log = pd.Series(indexedDataset[targetColm].ix[0],
                                          index=indexedDataset[targetColm].index)
        ...
        plt.title('RMSE: %.4f' % np.sqrt(
            sum((predictions_ARIMA_log - indexedDataset[targetColm]) ** 2) / len(indexedDataset[targetColm])))
        # plt.show()
